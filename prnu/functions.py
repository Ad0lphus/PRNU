...
def extract_single(im: np.ndarray,
    ...
    if denoiser == 'original':
        ...
        from prnu.VDNet.demo_test_benchmark import extract_noise_VDNet
        W = extract_noise_VDNet(im)
    elif denoiser == 'vdid':
        from prnu.VDIR.test import extract_noise_VDID
        W = extract_noise_VDID(im.astype(np.float32) /255.)
...
def define_param(denoiser_type = 'original',
                 remove_zero_mean: bool = True,
                 remove_wiener_filter: bool = True):
    global remove_zero_m
    remove_zero_m = remove_zero_mean
    global remove_wiener
    remove_wiener = remove_wiener_filter
    global denoiser
    denoiser = denoiser_type
    print('set denoiser type on:' + denoiser)
...
def noise_extract_compact(args):
    ...
    if denoiser == 'original':
        ...
        from prnu.VDNet.demo_test_benchmark import extract_noise_VDNet
        w = extract_noise_VDNet(args[0])
    elif denoiser == 'vdid':
        from prnu.VDIR.test import extract_noise_VDID
        w = extract_noise_VDID(args[0].astype(np.float32) /255.)
